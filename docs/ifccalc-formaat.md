...
- `history` (optioneel, sinds 2.2): wijzigingshistorie van de regel — per
  veldwijziging `{ field, oldValue, newValue, timestamp, user }` (de Windows-
  gebruiker die de wijziging deed). Snelle opeenvolgende bewerkingen van
  hetzelfde veld worden samengevoegd; maximaal 200 entries per regel.
...
Huidige formaatversie: **2.2.0** — één bron van waarheid in code:
...
| **2.1.0** | `spreadsheets`-object (bladen + actief blad), `projectInfo` (kengetallen) op schedule, `branches`-velden, `snapshots` | — |
| **2.2.0** *(huidig)* | `history` op CostItem: wijzigingshistorie per regel (veld, oud/nieuw, tijd, Windows-gebruiker) | — (additief; oudere bestanden hebben geen historie) |
