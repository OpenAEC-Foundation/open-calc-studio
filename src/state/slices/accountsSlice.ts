...
export interface AccountsSlice {
  ...
  /** Cloudmap waarin de begroting het laatst is opgeslagen (voor IFC-mapoverzicht). */
  lastCloudFolder: string | null;
...
export const createAccountsSlice: StateCreator<AccountsSlice> = (set, get) => ({
  ...
  cloudUploadCurrent: async (path?: string[], fileName?: string) => {
    ...
    set({ storageInfo: storage, lastCloudFolder: path && path.length ? path.join('/') : '' });
