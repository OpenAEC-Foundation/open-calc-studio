...
#let col-changed = rgb("#f8d9c0")   // gewijzigde regel: zacht oranje
#let show-changes = data.at("report_show_changes", default: false)
...
// Rapporttitel (getoond voor de directiebegroting-stijl)
#let report-title = data.at("report_title", default: "")
#if report-title != "" {
  v(2pt)
  align(center, text(size: 11pt, weight: "bold", report-title))
  v(3pt)
}

// ── Hoofdtabel kolommen ──
// Stabucode | S | Omschrijving | Hoeveelheid | Eh | Uurnorm | Uren | Materiaal | Materieel | Onderaan | Eenheidsprijs | TOTAAL
#let is-directie = data.at("style", default: "ibis") == "directie"
// De directiebegroting toont volledige hiërarchische STABU-codes (bv.
// "21.50.10-a.01>a") in de eerste kolom; die krijgt daarom flink meer breedte,
// zodat lange codes niet in de omschrijving overlopen.
#let col-widths = if is-directie {
  (70fr, 8fr, 196fr, 34fr, 20fr, 28fr, 28fr, 46fr, 46fr, 46fr, 48fr, 52fr)
} else {
  ...
  table(
    ...
    ..chapter.rows.map(row => {
      ...
      if row.is_subtotal {
        ...
        // normale regel — gewijzigde regels krijgen een zacht-oranje fill
        let row-fill = if show-changes and row.at("changed", default: false) { col-changed } else { none }
        (
          ..vals.enumerate().map(((idx, v)) => table.cell(
            fill: row-fill,
            fmt-cell(idx, v),
